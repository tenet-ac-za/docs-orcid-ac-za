---
title: "Manual Work"
url: "/hub/manual/writing_work_items"
sidebar_nav: "sidebar-hubmanual"
---
...
â€™Worksâ€™ is the generic term used by ORCID to describe research outputs. A large range of outputs is
...
| Invitee local identifier | A local identifier â€“ e.g. one used by your internal system. |
...
| Journal title | This is the title of the main body that the work is a part of â€“ can be journal title, book title, conference title etc. Note that ALL variations are captured under the term â€˜journal titleâ€™; do NOT change the name of this header.|
| Type (required) | The type of work â€“ for example: journal-article; book-chapter. Must be one of ORCIDâ€™s accepted types found in https://members.orcid.org/api/resources/worktypes |
...
| Citation type | Citations may be fielded as in RIS, BibTex (preferred by ORCID) â€“ or may be textual (APA, MLA, Chicago)|
...
| External ID Type (required) | The external ID type of the work; for example a DOI, ISBN etc. Must be one of ORCIDâ€™s accepted identifiers found in https://pub.orcid.org/v2.0/identifiers . If you are unable to assign a DOI or ISBN you can use â€˜source-work-idâ€™.|
| External ID relationship (required)| (json or yaml file must be used if more than one external relationship identifier is included) SELF (e.g. if your â€˜external ID typeâ€™ is, for example, a DOI for a journal article). You can also have a PART_OF identifier relationship such as the ISSN of the journal in which the article ispublished). A SELF identifier is mandatory.|
...
| Contributor-sequence | Can be â€˜firstâ€™ or â€˜additionalâ€™ only|
| PUT code | Use the PUT-Code if you wish to update an item. The code can be obtained by exporting the task from the Hub once the entry is written to the ORCID record. By writing information using a PUT code the row of information in your fileoverwrites the specified item in the userâ€™s ORCID record rather than creating a new entry.|
...
your file in the Hub, and according to your organisationâ€™s normal communications procedures, you
...
will be expecting the Hubâ€™s email and be less likely to delete it as spam.
To upload â€˜worksâ€™ file to the Hub log-in and go to
>Tasks >Upload works ...upload your fileâ€¦>activate all
This will either
...
## Editing records using the Hubâ€™s web-form feature
...
must â€˜resetâ€™ the task for the edits to appear in the records. If adding new invitees, these people will
